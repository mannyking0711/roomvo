...
const {data: products} = (await useFetch('/api/ext/all', {
  query: {
    vendor_id: store.extMeta.vendorId,
    product_id: store.extMeta.products[0].id,
...
watch(
    ...
    async (type: any) => {
      ...
      const {data} = await useFetch('/api/ext/all', {
        query: {
          vendor_id: store.extMeta.vendorId,
          product_id: store.extMeta.products[1].id,
          type: type.value
        }
      })
